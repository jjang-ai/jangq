...
// Bit extraction helpers (same as MXQDequant.metal)
...
kernel void mxq_embedding_dequant(
...
//
// We use mxq_dequant_gemv for this — each threadgroup computes one
// output element (one vocab logit) by dequantizing one row of the
// embedding table and dotting it with the hidden state.
//
// This kernel is identical to mxq_dequant_gemv but is here for clarity
...
//
// Use mxq_dequant_gemv directly with:
