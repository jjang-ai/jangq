...
public final class MXQTokenizer {
    // Core tokenizer data
    ...
    public init(tokenizerPath: URL) throws {
        ...
        guard let config = try JSONSerialization.jsonObject(with: configData) as? [String: Any] else {
            throw MXQError.tokenizerError("Invalid tokenizer_config.json")
        }
        ...
        guard let tokJson = try JSONSerialization.jsonObject(with: tokData) as? [String: Any] else {
            throw MXQError.tokenizerError("Invalid tokenizer.json")
        }

        guard let model = tokJson["model"] as? [String: Any] else {
            throw MXQError.tokenizerError("Missing 'model' in tokenizer.json")
        }

        // Load vocab
        guard let vocabDict = model["vocab"] as? [String: Int] else {
            throw MXQError.tokenizerError("Missing vocab in tokenizer model")
        }
        ...
        guard let mergesList = model["merges"] as? [String] else {
            throw MXQError.tokenizerError("Missing merges in tokenizer model")
        }
