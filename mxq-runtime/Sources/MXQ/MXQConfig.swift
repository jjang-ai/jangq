...
public struct MXQQuantConfig: Sendable {
    ...
    public init(from dict: [String: Any]) throws {
        guard let format = dict["format"] as? String, format == "mxq" else {
            throw MXQError.invalidFormat("format field must be 'mxq'")
        }

        self.formatVersion = dict["format_version"] as? String ?? "1.0"

        guard let quant = dict["quantization"] as? [String: Any] else {
            throw MXQError.invalidFormat("missing 'quantization' section")
        }
...
public struct MXQModelConfig: Sendable {
    public let model: ModelConfig
    public let quant: MXQQuantConfig
    public let modelPath: URL

    public static func load(from path: URL) throws -> MXQModelConfig {
        // Load config.json
        ...
        guard let mxqDict = try JSONSerialization.jsonObject(with: mxqData) as? [String: Any] else {
            throw MXQError.invalidFormat("mxq_config.json is not a valid JSON object")
        }
        let quant = try MXQQuantConfig(from: mxqDict)

        return MXQModelConfig(model: model, quant: quant, modelPath: path)
    }
}
