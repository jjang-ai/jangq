...
> **Published bundle warning:** re-download Osaurus/JANGQ Gemma 4 12B bundles if pulled before **2026-06-03 3:19 PM PDT**. The current model cards and templates include both fixes: OpenAI `tool_choice: "required"` is encoded in the native Gemma tool path, and normal no-thinking multi-turn chat no longer pre-seeds an empty `<|channel>thought\n<channel|>` block.
...
- Current JANG/Osaurus bundles patch `add_generation_prompt` to append only `<|turn>model\n` for normal generation. No `<|channel>thought` marker is prefilled when `enable_thinking` is false or missing.
...
- **Disabled-thinking behavior:** do not pre-seed an empty thought block. The safe no-thinking generation tail is just `<|turn>model\n`; adding `<|channel>final` is not part of the verified Gemma 4 12B template contract.
...
| tied `embed_tokens.weight` (2D) | **fp16 passthrough** (used as both embedding + output via `as_linear`; no `lm_head` tensor) |
...
- [ ] Register the patched `gemma4` chat template: turns, explicit thought channel only when thinking/reasoning is active, and no empty-thought prefill on no-think generation. (§10.1-10.2)
