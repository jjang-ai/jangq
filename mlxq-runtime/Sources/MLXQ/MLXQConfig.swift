...
/// MXQ quantization configuration (from mxq_config.json).
public struct MLXQQuantConfig: Sendable {
    ...
    public init(from dict: [String: Any]) throws {
        guard let format = dict["format"] as? String, format == "mxq" else {
            throw MLXQError.invalidFormat("format field must be 'mxq'")
        }

        self.formatVersion = dict["format_version"] as? String ?? "1.0"

        guard let quant = dict["quantization"] as? [String: Any] else {
            throw MLXQError.invalidFormat("missing 'quantization' section")
        }
...
public struct MLXQModelConfig: Sendable {
    public let model: ModelConfig
    public let quant: MLXQQuantConfig
    public let modelPath: URL

    public static func load(from path: URL) throws -> MLXQModelConfig {
        // Load config.json
        ...
        // Load mxq_config.json
        let mxqConfigURL = path.appendingPathComponent("mxq_config.json")
        ...
        guard let mxqDict = try JSONSerialization.jsonObject(with: mxqData) as? [String: Any] else {
            throw MLXQError.invalidFormat("mxq_config.json is not a valid JSON object")
        }
        let quant = try MLXQQuantConfig(from: mxqDict)

        return MLXQModelConfig(model: model, quant: quant, modelPath: path)
    }
}
