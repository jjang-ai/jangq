...
public struct MLXQWeight: @unchecked Sendable {
...
public struct TransformerLayerWeights: @unchecked Sendable {
    ...
    // Attention
    public let qProj: MLXQWeight
    public let kProj: MLXQWeight
    public let vProj: MLXQWeight
    public let oProj: MLXQWeight
    ...
    // MLP
    public let gateProj: MLXQWeight
    public let upProj: MLXQWeight
    public let downProj: MLXQWeight
...
public struct MXQModel: @unchecked Sendable {
    public let config: MLXQModelConfig
    public let embedTokens: MLXQWeight        // embedding weight (quantized)
    public let finalNorm: MTLBuffer          // final RMSNorm weight
    public let lmHead: MLXQWeight?            // output head (nil if tied with embeddings)
...
public func loadModel(url: URL, device: MTLDevice) throws -> MXQModel {
    ...
    // 1. Load configs
    let config = try MLXQModelConfig.load(from: url)
    ...
    let embedTokens = try loadMLXQWeight(named: "model.embed_tokens",
    ...
    var lmHead: MLXQWeight? = nil
    if !modelConfig.tiedEmbeddings {
        if hasMLXQWeight(named: "lm_head", in: allTensors) {
            lmHead = try loadMLXQWeight(named: "lm_head", from: allTensors, device: device)
        }
    }
    ...
    for i in 0..<modelConfig.numHiddenLayers {
        ...
        let layer = try TransformerLayerWeights(
            index: i,
            qProj: loadMLXQWeight(named: "\(prefix).self_attn.q_proj",
                                  from: allTensors, device: device),
            kProj: loadMLXQWeight(named: "\(prefix).self_attn.k_proj",
                                  from: allTensors, device: device),
            vProj: loadMLXQWeight(named: "\(prefix).self_attn.v_proj",
                                  from: allTensors, device: device),
            oProj: loadMLXQWeight(named: "\(prefix).self_attn.o_proj",
                                  from: allTensors, device: device),
            qBias: qBias,
            kBias: kBias,
            vBias: vBias,
            gateProj: loadMLXQWeight(named: "\(prefix).mlp.gate_proj",
                                     from: allTensors, device: device),
            upProj: loadMLXQWeight(named: "\(prefix).mlp.up_proj",
                                   from: allTensors, device: device),
            downProj: loadMLXQWeight(named: "\(prefix).mlp.down_proj",
                                     from: allTensors, device: device),
...
private func hasMLXQWeight(named baseName: String, in index: TensorIndex) -> Bool {
    return index["\(baseName).qweight"] != nil
}

private func loadMLXQWeight(named baseName: String,
                            from index: TensorIndex,
                            device: MTLDevice) throws -> MLXQWeight {
    ...
    guard let (qShard, _) = index[qweightName] else {
        throw MLXQError.tensorNotFound(qweightName)
    }
    ...
    guard let (_, bitMapInfo) = index[bitMapName] else {
        throw MLXQError.tensorNotFound(bitMapName)
    }
...
private func loadBuffer(named name: String,
    ...
    guard let (shard, _) = index[name] else {
        throw MLXQError.tensorNotFound(name)
    }
