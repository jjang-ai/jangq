...
public final class MLXQTokenizer {
    // Core tokenizer data
    ...
    public init(tokenizerPath: URL) throws {
        ...
        guard let config = try JSONSerialization.jsonObject(with: configData) as? [String: Any] else {
            throw MLXQError.tokenizerError("Invalid tokenizer_config.json")
        }
        ...
        guard let tokJson = try JSONSerialization.jsonObject(with: tokData) as? [String: Any] else {
            throw MLXQError.tokenizerError("Invalid tokenizer.json")
        }

        guard let model = tokJson["model"] as? [String: Any] else {
            throw MLXQError.tokenizerError("Missing 'model' in tokenizer.json")
        }

        // Load vocab
        guard let vocabDict = model["vocab"] as? [String: Int] else {
            throw MLXQError.tokenizerError("Missing vocab in tokenizer model")
        }
        ...
        guard let mergesList = model["merges"] as? [String] else {
            throw MLXQError.tokenizerError("Missing merges in tokenizer model")
        }
