...
@main
struct MLXQCLI: ParsableCommand {
...
struct Run: ParsableCommand {
    ...
    func run() throws {
        ...
        let metalDevice = try MLXQMetalDevice()
        ...
        let tokenizer = try MLXQTokenizer(tokenizerPath: tokenizerPath)

        // 4. Initialize inference engine
        print("  Initializing inference engine...")
        let engine = try MLXQInferenceEngine(
            model: model,
...
struct Debug: ParsableCommand {
    ...
    func run() throws {
        ...
        let model = try loadModel(url: url, device: metalDevice.device)
        let engine = try MLXQInferenceEngine(model: model, metalDevice: metalDevice, maxSeqLen: 128)
