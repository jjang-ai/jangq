"""
MXQ Calibration Engine — Compute importance scores from real model weights.
...
foundation that makes MXQ's mixed-precision allocation work.
...
def calibrate_from_weights(
    ...
    Args:
        ...
        output_path: where to save imatrix (default: model_path/mxq_imatrix.safetensors)
    ...
    for sf_path in tqdm(weight_files, desc="  Loading weights"):
        with safe_open(str(sf_path), framework="numpy") as f:
            for tensor_name in f.keys():
                ...
                # Load tensor — handle bfloat16 by reading raw bytes
                try:
                    tensor = f.get_tensor(tensor_name)
                except TypeError:
                    # bfloat16 not supported by numpy — read raw bytes and convert
                ...
                # Apply architecture-specific importance weight
                layer_config = get_layer_config(arch_config, tensor_name)
                importance = raw_score * layer_config.importance_weight

                # Normalize to [0, 1] within this tensor
    ...
    if output_path is None:
        output_path = model_path / "mxq_imatrix.safetensors"
