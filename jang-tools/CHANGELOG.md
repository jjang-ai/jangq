...
- Gemma 4 12B sidecar handling now patches `chat_template.jinja` before writing
  bundles: it injects the native required-tool stanza for
  `tool_choice == "required"` and removes the bad no-thinking generation tail
  that prefilled `<|channel>thought\n<channel|>`. The patched template is always
  folded into `tokenizer_config.json["chat_template"]` so file and embedded
  forms match.
- Registered `gemma4_unified` / `gemma4_unified_text` in the capability
  ...
  fallback).
- Added `scripts/gemma4/` validation tools: `g4_coherence.py` (text-decoder
  coherence via a `gemma4_text` shim), `verify_bundle_integrity.py` (multimodal/
  tokenizer/audio faithfulness + fp16 range safety), `validate_dequant.py`
  (dequant round-trip vs source). Runtime contract:
  `docs/runtime/2026-06-03-gemma4-12b-unified-runtime-spec.md`.
