...
def measure_lm_head(bundle: Path | str) -> tuple[int, int, str, bool]:
    ...
    if idx.exists():
        ...
        if wname is not None:
            ...
            if w is not None and s is not None and w["dtype"] == "U8" and gs == 128:
                # JANG ternary_packed_26b storage (Bonsai 2): uint8
                # [rows, groups*26] trits expanded to 2-bit slots at load.
                # 26 bytes per 128-weight group is the whole contract.
                if w["shape"][-1] == s["shape"][-1] * 26 and bits == 2:
                    return bits, gs, mode, tied
                raise HeadLayoutMismatch(
                    f"{bundle.name}: uint8 lm_head shard {w['shape']} is not "
                    f"ternary_packed_26b for declared {bits}b/g{gs}")
