"""Convert Zyphra/ZAYA1-VL-8B BF16 weights to a ZAYA1-VL JANGTQ bundle."""
...
from jang_tools.convert_zaya1_vl_common import (
    copy_zaya1_vl_sidecars,
    zaya1_vl_capabilities,
    is_passthrough,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert Zyphra/ZAYA1-VL-8B BF16 source to JANGTQ."
    )
    parser.add_argument("src", type=Path, help="Local Zyphra/ZAYA1-VL-8B BF16 directory")
...
def main() -> None:
    ...
    if expert_bits == 3:
        raise SystemExit("JANGTQ3 is intentionally unsupported for ZAYA/VL; use JANGTQ2 or JANGTQ4")

    try:
        config = load_json(src / "config.json")
        text_cfg = config.get("text_config", config)
        hidden_size = int(text_cfg["hidden_size"])
        n_experts = int(text_cfg.get("num_experts", text_cfg.get("num_local_experts", 8)))

        print("=" * 70)
        print(f"  Zyphra/ZAYA1-VL-8B -> {profile} conversion")
        print("=" * 70)
        print(f"  Source: {src}")
        print(f"  Output: {out}")
        ...
        for layer in tqdm(layers, desc="  Experts"):
            members = {
                expert: experts[(layer, expert)]
                for expert in range(n_experts)
                if (layer, expert) in experts
            }
        ...
        config["capabilities"] = zaya1_vl_capabilities()
        ...
        copy_zaya1_vl_sidecars(src, out)
