...
_WEIGHTED_GATHER_TQ_SOURCE = f'''
    ...
    float weighted[{_GATHER_OPT}];
    #pragma unroll
    for (uint o = 0; o < {_GATHER_OPT}; o++) weighted[o] = 0.0f;

    uint n_outs = {_GATHER_OPT}u;
    if (out_idx_0 + {_GATHER_OPT}u > out_features) n_outs = out_features - out_idx_0;

    for (uint k_idx = 0; k_idx < K; k_idx++) {{
        uint expert = rhs_indices[k_idx];
        float score = static_cast<float>(scores[k_idx]);
        uint expert_base = expert * out_features * packed_cols;
        uint x_offset = k_idx * in_features;
...
def _get_weighted_kernel():
    if "weighted_gather_tq" not in _kernel_cache:
        _kernel_cache["weighted_gather_tq"] = mx.fast.metal_kernel(
            name="weighted_gather_tq_matmul",
            input_names=["x_rot", "packed", "norms", "codebook", "rhs_indices", "scores", "meta"],
            output_names=["out"],
            source=_WEIGHTED_GATHER_TQ_SOURCE,
        )
    return _kernel_cache["weighted_gather_tq"]
...
def make_weighted_gather_tq_decode_per_row(in_features, out_features, bits, K):
    """Return fn(x_rot, packed, norms, cb, idx_flat, scores_flat) -> (1, out_f).

    This decode-only helper fuses the final top-k weighted sum into the down
    projection gather. It avoids materializing ``(K, out_f)`` and then launching
    a separate multiply/reduce for SwitchGLU's weighted MoE output.
    """
    key = ("weighted_gather_per_row", in_features, out_features, bits, K)
    if key in _DECODE_CACHE:
        return _DECODE_CACHE[key]
    vals_per_u32 = 32 // bits
    packed_cols = (in_features + vals_per_u32 - 1) // vals_per_u32
    ...
    grid_x = out_groups * 32
    tg_x = min(grid_x, 256)
    kernel = _get_weighted_kernel()

    def _fn(x_rot, packed, norms, cb, idx_flat, scores_flat):
        out, = kernel(
            inputs=[x_rot, packed, norms, cb, idx_flat, scores_flat, meta],
...
def make_gather_tq_decode_broadcast(in_features, out_features, bits, K):
    """Return pure function(x_rot, packed, norms, cb, idx_flat) -> (K, out_f).

    x_rot: (1, in_features) float32, already Hadamard-rotated.
    idx_flat: (K,) uint32.

    Broadcast mode uses one input row for all K selected experts. This matches
    SwitchMLP/SwitchGLU fc1/gate/up decode semantics without materializing
    ``mx.repeat(x_rot, K, axis=0)`` before the gather kernel.
    """
    key = ("gather_broadcast", in_features, out_features, bits, K)
    if key in _DECODE_CACHE:
        return _DECODE_CACHE[key]
    vals_per_u32 = 32 // bits
    packed_cols = (in_features + vals_per_u32 - 1) // vals_per_u32
