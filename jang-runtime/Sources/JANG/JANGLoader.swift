 ...
 *
 * Loading pipeline:
 *   1. Parse jang_config.json + config.json
 *   2. Memory-map all .jang.safetensors shard files
...
public struct JANGWeight: @unchecked Sendable {
...
public struct TransformerLayerWeights: @unchecked Sendable {
    ...
    // Attention
    public let qProj: JANGWeight
    public let kProj: JANGWeight
    public let vProj: JANGWeight
    public let oProj: JANGWeight
    ...
    // MLP
    public let gateProj: JANGWeight
    public let upProj: JANGWeight
    public let downProj: JANGWeight
...
public struct MXQModel: @unchecked Sendable {
    ...
    public let embedTokens: JANGWeight        // embedding weight (quantized)
    public let finalNorm: MTLBuffer          // final RMSNorm weight
    public let lmHead: JANGWeight?            // output head (nil if tied with embeddings)
...
public func loadModel(url: URL, device: MTLDevice) throws -> MXQModel {
    ...
    let embedTokens = try loadJANGWeight(named: "model.embed_tokens",
    ...
    var lmHead: JANGWeight? = nil
    if !modelConfig.tiedEmbeddings {
        if hasJANGWeight(named: "lm_head", in: allTensors) {
            lmHead = try loadJANGWeight(named: "lm_head", from: allTensors, device: device)
        }
    }
    ...
    for i in 0..<modelConfig.numHiddenLayers {
        ...
        let layer = try TransformerLayerWeights(
            index: i,
            qProj: loadJANGWeight(named: "\(prefix).self_attn.q_proj",
                                  from: allTensors, device: device),
            kProj: loadJANGWeight(named: "\(prefix).self_attn.k_proj",
                                  from: allTensors, device: device),
            vProj: loadJANGWeight(named: "\(prefix).self_attn.v_proj",
                                  from: allTensors, device: device),
            oProj: loadJANGWeight(named: "\(prefix).self_attn.o_proj",
                                  from: allTensors, device: device),
            qBias: qBias,
            kBias: kBias,
            vBias: vBias,
            gateProj: loadJANGWeight(named: "\(prefix).mlp.gate_proj",
                                     from: allTensors, device: device),
            upProj: loadJANGWeight(named: "\(prefix).mlp.up_proj",
                                   from: allTensors, device: device),
            downProj: loadJANGWeight(named: "\(prefix).mlp.down_proj",
                                     from: allTensors, device: device),
...
private func hasJANGWeight(named baseName: String, in index: TensorIndex) -> Bool {
    return index["\(baseName).qweight"] != nil
}

private func loadJANGWeight(named baseName: String,
                            from index: TensorIndex,
                            device: MTLDevice) throws -> JANGWeight {
    ...
    guard let (qShard, _) = index[qweightName] else {
        throw JANGError.tensorNotFound(qweightName)
    }
    ...
    guard let (_, bitMapInfo) = index[bitMapName] else {
        throw JANGError.tensorNotFound(bitMapName)
    }
...
private func loadBuffer(named name: String,
    ...
    guard let (shard, _) = index[name] else {
        throw JANGError.tensorNotFound(name)
    }
