...
public final class JANGInferenceEngine {
    private let metalDevice: JANGMetalDevice
    ...
    public init(model: MXQModel, metalDevice: JANGMetalDevice, maxSeqLen: Int = 2048) throws {
        self.model = model
    ...
    private func makeCmd() throws -> MTLCommandBuffer {
        guard let cmd = metalDevice.commandQueue.makeCommandBuffer() else {
            throw JANGError.inferenceError("Failed to create command buffer")
        }
        return cmd
    }
    ...
    private func dispatchEmbedding(cmdBuffer: MTLCommandBuffer, tokenId: Int) throws {
        guard let encoder = cmdBuffer.makeComputeCommandEncoder() else {
            throw JANGError.inferenceError("Failed to create compute encoder")
        }

        let embed = model.embedTokens
        let pipeline = try metalDevice.pipeline(for: "jang_embedding_dequant")
    ...
    private func dispatchRMSNorm(cmdBuffer: MTLCommandBuffer,
        ...
        let pipeline = try metalDevice.pipeline(for: "jang_rms_norm")
        encoder.setComputePipelineState(pipeline)
    ...
    private func dispatchDequantGEMV(cmdBuffer: MTLCommandBuffer,
                                      input: MTLBuffer, weight: JANGWeight,
                                      output: MTLBuffer,
                                      K: Int, N: Int) throws {
        guard let encoder = cmdBuffer.makeComputeCommandEncoder() else {
            throw JANGError.inferenceError("Failed to create compute encoder")
        ...
        let pipeline = try metalDevice.pipeline(for: "jang_dequant_gemv")
        encoder.setComputePipelineState(pipeline)
    ...
    private func dispatchRoPE(cmdBuffer: MTLCommandBuffer,
        ...
        let pipeline = try metalDevice.pipeline(for: "jang_rope")
        encoder.setComputePipelineState(pipeline)
    ...
    private func storeKVCache(cmdBuffer: MTLCommandBuffer, layer: Int) throws {
        ...
        guard let blitEncoder = cmdBuffer.makeBlitCommandEncoder() else {
            throw JANGError.inferenceError("Failed to create blit encoder")
        }
    ...
    private func dispatchAttention(cmdBuffer: MTLCommandBuffer, layer: Int) throws {
        ...
        let pipeline = try metalDevice.pipeline(for: "jang_attention_decode")
        encoder.setComputePipelineState(pipeline)
    ...
    private func dispatchAddInPlace(cmdBuffer: MTLCommandBuffer,
        ...
        // Use jang_add: output = a + b, with output == buffer (in-place via temp)
        // Actually we need in-place add. Let's use a simple approach:
        // Read buffer into temp, add bias, write back.
        // But we can just use jang_add with buffer as both input a and output,
        // since Metal processes all threads before writing.
        // Wait — that's not safe. Let's dispatch jang_add with buffer as 'a',
        ...
        let pipeline = try metalDevice.pipeline(for: "jang_add")
        encoder.setComputePipelineState(pipeline)
    ...
    private func dispatchSiLUMul(cmdBuffer: MTLCommandBuffer,
        ...
        let pipeline = try metalDevice.pipeline(for: "jang_silu_mul")
        encoder.setComputePipelineState(pipeline)
    ...
    private static func makeBuffer(_ device: MTLDevice, _ size: Int) throws -> MTLBuffer {
        guard let buffer = device.makeBuffer(length: size, options: .storageModeShared) else {
            throw JANGError.bufferAllocationFailed(size)
        }
        return buffer
    }
}
