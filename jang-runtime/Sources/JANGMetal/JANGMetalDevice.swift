...
public final class JANGMetalDevice: @unchecked Sendable {
    ...
    public init() throws {
        guard let device = MTLCreateSystemDefaultDevice() else {
            throw JANGError.metalNotAvailable
        }
        self.device = device
    ...
    public func pipeline(for kernelName: String) throws -> MTLComputePipelineState {
        ...
        guard let function = library.makeFunction(name: kernelName) else {
            throw JANGError.kernelNotFound(kernelName)
        }
    ...
    private static func loadMetalLibrary(device: MTLDevice) throws -> MTLLibrary {
        ...
        let searchPaths: [String] = [
            ...
            URL(fileURLWithPath: CommandLine.arguments[0])
                ...
                .appendingPathComponent("jang.metallib").path,
            // In the build directory
            URL(fileURLWithPath: CommandLine.arguments[0])
                .deletingLastPathComponent()
                .deletingLastPathComponent()
                .appendingPathComponent("Metal/jang.metallib").path,
            // Current working directory
            FileManager.default.currentDirectoryPath + "/jang.metallib",
            // Project Metal directory
            FileManager.default.currentDirectoryPath + "/Metal/jang.metallib",
        ]
        ...
        throw JANGError.metallibNotFound
    }
    ...
    private func precompilePipelines() throws {
        let kernelNames = [
            ...
            "jang_add",
            "jang_embedding",
            "jang_embedding_dequant",
            "jang_attention_decode",
            "jang_attention_prefill",
        ]
